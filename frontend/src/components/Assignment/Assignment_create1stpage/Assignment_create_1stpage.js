...
import "./Assignment_create_1stpage.css";
...
import AssignmentOptions from "../assignment_create/AssignmentOptions";
import { useDispatch, useSelector } from "react-redux";
import Cookies from "js-cookie";
import { createAssessment } from "../../../redux/features/dashboardSlices/assessmentSlice";
import { useNavigate } from "react-router-dom";

export default function Assignment_create_1stpage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if(!section || !course ||  !marks || !asType || !title || !instructions || !date || !selectedFile || selectedFile.name === "No file selected") {
      return
    }

    
        const fileRes = await fetch(`https://www.filestackapi.com/api/store/S3?key=AlLZdDNYoRrmtnyMDwCgyz`, {
          method: 'POST',
          body :  selectedFile
        })

        const fileJson =  await fileRes.json()
        console.log("fileJson", fileJson.url)

        // const response = await fetch(`${process.env.REACT_APP_LMS_BACKEND_API}/assessment/assignment/create`, {
        //     method: 'POST',
        //     headers: {'Content-Type' : 'application/json', 'Authorization': `Bearer ${Cookies.get("token")}`},
        //     body: JSON.stringify({title, description: instructions, course, section, type: asType, marks, dueDate: "2023-05-20T00:00:00.000Z", file: fileJson.url, createdBy : Cookies.get('userId') }) 
        // })
